  ...
  return (
    <section id="admin-management-section" className="pt-2">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        {/* Buttons Toolbar - Responsive Grid Layout */}
        <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-8 gap-3 items-stretch">
            
            {/* NEW BUTTON: Opportunity Master Admin Tool */}
            <button
              id="btn-admin-open-opportunity-admin"
              type="button"
              onClick={() => setIsOpportunityAdminOpen(true)}
              className="group relative flex items-center justify-between h-12 px-3 bg-white hover:bg-indigo-50/40 border border-slate-200 hover:border-indigo-300 rounded-xl shadow-2xs transition-all cursor-pointer text-left"
            >
              <div className="flex items-center min-w-0">
                <div className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-100 mr-2 shrink-0 group-hover:scale-105 transition-transform">
                  <Layers className="w-4 h-4" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs sm:text-sm font-bold text-slate-800 group-hover:text-indigo-700 truncate leading-tight">
                    Opportunity Admin
                  </div>
              ...
              <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 shrink-0 ml-1">
                {opportunities.length}
              </span>
            </button>

            {/* NEW BUTTON: Finance Admin Tool */}
            <button
              id="btn-admin-open-finance-admin"
              type="button"
              onClick={() => setIsFinanceAdminOpen(true)}
              className="group relative flex items-center justify-between h-12 px-3 bg-white hover:bg-purple-50/40 border border-slate-200 hover:border-purple-300 rounded-xl shadow-2xs transition-all cursor-pointer text-left"
            >
              <div className="flex items-center min-w-0">
                <div className="p-1.5 rounded-lg bg-purple-50 text-purple-600 border border-purple-100 mr-2 shrink-0 group-hover:scale-105 transition-transform">
                  <Calculator className="w-4 h-4" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs sm:text-sm font-bold text-slate-800 group-hover:text-purple-700 truncate leading-tight">
                    Finance Admin
                  </div>
                  <div className="text-[10px] text-slate-500 font-medium truncate">
                    Approval Thresholds
              ...
              <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-purple-50 text-purple-700 border border-purple-200 shrink-0 ml-1">
                {financeConfig?.tiers?.length || 4} Tiers
              </span>
            </button>

            {/* BUTTON 1: Resource Directory with Sub-Actions */}
            <div className="group relative flex items-stretch h-12 bg-white hover:bg-indigo-50/30 border border-slate-200 hover:border-indigo-300 rounded-xl shadow-2xs transition-all overflow-hidden">
              <button
                id="btn-admin-open-resource-directory"
                type="button"
                onClick={() => setIsResourceDirectoryOpen(true)}
                className="flex-1 flex items-center min-w-0 pl-3 pr-2 py-1 text-left cursor-pointer hover:bg-indigo-50/50 transition-colors"
              >
                <div className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-100 mr-2 shrink-0 group-hover:scale-105 transition-transform">
              ...
              <div className="h-6 self-center w-px bg-slate-200 shrink-0" />

              {/* Sub-actions: Add & Bulk */}
              <div className="flex items-center px-1 bg-slate-50/60 shrink-0 space-x-0.5">
                <button
                  id="btn-admin-quick-add-resource"
                  type="button"
                  onClick={() => setIsQuickAddResourceOpen(true)}
                  title="Add New Resource Member"
                  className="p-1.5 rounded-lg text-slate-600 hover:text-indigo-600 hover:bg-indigo-100 transition-colors cursor-pointer"
                >
            ...
            {/* BUTTON 2: Client / Org Directory with Sub-Actions */}
            <div className="group relative flex items-stretch h-12 bg-white hover:bg-blue-50/30 border border-slate-200 hover:border-blue-300 rounded-xl shadow-2xs transition-all overflow-hidden">
              <button
                id="btn-admin-open-client-directory"
                type="button"
                onClick={() => setIsDirectoryModalOpen(true)}
                className="flex-1 flex items-center min-w-0 pl-3 pr-2 py-1 text-left cursor-pointer hover:bg-blue-50/50 transition-colors"
              >
                <div className="p-1.5 rounded-lg bg-blue-50 text-blue-600 border border-blue-100 mr-2 shrink-0 group-hover:scale-105 transition-transform">
                  <Building2 className="w-4 h-4" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-xs sm:text-sm font-bold text-slate-800 group-hover:text-blue-700 truncate leading-tight">
                    Client Directory
                  </div>
                  <div className="text-[10px] text-slate-500 font-medium truncate">
              ...
              <div className="flex items-center px-1 bg-slate-50/60 shrink-0 space-x-0.5">
                <button
                  ...
                  className="p-1.5 rounded-lg text-slate-600 hover:text-blue-600 hover:bg-blue-100 transition-colors cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
                </button>
                <button
            ...
            {/* BUTTON 3: Target SLA Admin */}
            <button
              id="btn-admin-open-target-slas"
              type="button"
              onClick={() => setIsTargetSlaAdminOpen(true)}
              className="group relative flex items-center justify-between h-12 px-3 bg-white hover:bg-blue-50/40 border border-slate-200 hover:border-blue-300 rounded-xl shadow-2xs transition-all cursor-pointer text-left"
            >
              <div className="flex items-center min-w-0">
                <div className="p-1.5 rounded-lg bg-blue-50 text-blue-600 border border-blue-100 mr-2 shrink-0 group-hover:scale-105 transition-transform">
            ...
            >
              ...
              <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-purple-50 text-purple-700 border border-purple-200 shrink-0 ml-1 flex items-center space-x-1">
                <Sparkles className="w-2.5 h-2.5 text-purple-500" />
                <span>Live Sync</span>
              </span>
            </button>

            {/* BUTTON 5: Export Master Data Menu */}
            <div className="relative">
              <button
                id="btn-admin-export-data"
                type="button"
                onClick={() => setShowExportMenu(!showExportMenu)}
                className="group relative w-full flex items-center justify-between h-12 px-3 bg-white hover:bg-emerald-50/40 border border-slate-200 hover:border-emerald-300 rounded-xl shadow-2xs transition-all cursor-pointer text-left"
              >
                <div className="flex items-center min-w-0">
                  <div className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100 mr-2 shrink-0 group-hover:scale-105 transition-transform">
                    <Download className="w-4 h-4" />
                  </div>
                  <div className="min-w-0">
                    <div className="text-xs sm:text-sm font-bold text-slate-800 group-hover:text-emerald-700 truncate leading-tight">
                      Export Data
                    </div>
                    <div className="text-[10px] text-slate-500 font-medium truncate">
                ...
                <div className="flex items-center space-x-1 shrink-0 ml-1">
                  ...
                  <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${showExportMenu ? 'rotate-180 text-emerald-600' : ''}`} />
                </div>
              </button>

              {/* Export Dropdown Options */}
              {showExportMenu && (
                <div className="absolute left-0 top-full mt-1.5 w-72 bg-white rounded-xl shadow-xl border border-slate-200 p-1.5 z-30 animate-in fade-in zoom-in-95 duration-100">
                  <button
                    ...
                    className="w-full px-3 py-2 text-left text-xs font-medium text-slate-700 hover:bg-emerald-50 hover:text-emerald-800 rounded-lg transition-colors flex items-center space-x-2.5 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 text-left text-xs font-medium text-slate-700 hover:bg-indigo-50 hover:text-indigo-800 rounded-lg transition-colors flex items-center space-x-2.5 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 text-left text-xs font-medium text-slate-700 hover:bg-blue-50 hover:text-blue-800 rounded-lg transition-colors flex items-center space-x-2.5 cursor-pointer"
                  >
            ...
            {/* BUTTON 6: Reset Demo Dataset */}
            <button
              id="btn-admin-reset-demo"
              type="button"
              onClick={() => setShowResetConfirm(true)}
              className="group relative flex items-center justify-between h-12 px-3 bg-white hover:bg-red-50/50 border border-slate-200 hover:border-red-200 rounded-xl shadow-2xs transition-all cursor-pointer text-left"
            >
              <div className="flex items-center min-w-0">
                <div className="p-1.5 rounded-lg bg-slate-100 group-hover:bg-red-100 text-slate-500 group-hover:text-red-600 border border-slate-200 group-hover:border-red-200 mr-2 shrink-0 transition-colors">
                  <RotateCcw className="w-4 h-4" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs sm:text-sm font-semibold text-slate-700 group-hover:text-red-700 truncate leading-tight">
                    Reset Demo Data
                  </div>
                  <div className="text-[10px] text-slate-400 group-hover:text-red-500 truncate font-medium">
                    Factory Defaults
                  </div>
                </div>
              </div>
              <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded bg-slate-100 group-hover:bg-red-100 text-slate-500 group-hover:text-red-700 transition-colors shrink-0 ml-1">
                Baseline
              </span>
            </button>
